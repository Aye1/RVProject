...
{ =============================================================================================== }
{
  ...
  or libfmod-3.63.so (in Linux) installed.
  ...
  In Linux, make sure you are signed in as root and copy the libfmod-3.63.so
  ...
  a symbolic link between libfmod-3.63.so and libfmod.so. This is done with
  the following command (assuming you are in /usr/lib/)...
  ln -s libfmod-3.63.so libfmod.so.
}
{ =============================================================================================== }
...
function FSOUND_Stream_OpenFile(const Filename: PChar; Mode: Cardinal; MemLength: Integer): PFSoundStream; {$IFDEF LINUX} cdecl {$ELSE} stdcall {$ENDIF};
...
function FSOUND_Stream_SetSynchCallback(Stream: PFSoundStream; Callback: TFSoundStreamCallback; UserData: Integer): ByteBool; {$IFDEF LINUX} cdecl {$ELSE} stdcall {$ENDIF};
function FSOUND_Stream_AddSynchPoint(Stream: PFSoundStream; PCMOffset: Integer; UserData: Integer): Integer; {$IFDEF LINUX} cdecl {$ELSE} stdcall {$ENDIF};
function FSOUND_Stream_DeleteSynchPoint(Stream: PFSoundStream; Index: Integer): ByteBool; {$IFDEF LINUX} cdecl {$ELSE} stdcall {$ENDIF};
function FSOUND_Stream_GetNumSynchPoints(Stream: PFSoundStream): Integer; {$IFDEF LINUX} cdecl {$ELSE} stdcall {$ENDIF};
...
function FMUSIC_LoadSongEx(Name_Or_Data: Pointer; MemLength: Integer; Mode: Cardinal; var SampleList: Integer; SampleListNum: Integer): PFMusicModule; {$IFDEF LINUX} cdecl {$ELSE} stdcall {$ENDIF};
...
function FSOUND_Sample_Load; external FMOD_DLL {$IFDEF MSWINDOWS} name '_FSOUND_Sample_Load@16' {$ENDIF};
...
function FSOUND_Stream_OpenFile; external FMOD_DLL {$IFDEF MSWINDOWS} name '_FSOUND_Stream_OpenFile@12' {$ENDIF};
...
function FSOUND_Stream_SetSynchCallback; external FMOD_DLL {$IFDEF MSWINDOWS} name '_FSOUND_Stream_SetSynchCallback@12' {$ENDIF};
...
function FSOUND_Stream_AddSynchPoint; external FMOD_DLL {$IFDEF MSWINDOWS} name '_FSOUND_Stream_AddSynchPoint@12' {$ENDIF};
function FSOUND_Stream_DeleteSynchPoint; external FMOD_DLL {$IFDEF MSWINDOWS} name '_FSOUND_Stream_DeleteSynchPoint@8' {$ENDIF};
function FSOUND_Stream_GetNumSynchPoints; external FMOD_DLL {$IFDEF MSWINDOWS} name '_FSOUND_Stream_GetNumSynchPoints@4' {$ENDIF};
...
function FMUSIC_LoadSongEx; external FMOD_DLL {$IFDEF MSWINDOWS} name '_FMUSIC_LoadSongEx@20' {$ENDIF};
