...
var
  ...
  FSOUND_Stream_OpenFile: function(const Filename: PChar; Mode: Cardinal; MemLength: Integer): PFSoundStream; {$IFDEF LINUX} cdecl {$ELSE} stdcall {$ENDIF};
  ...
  FSOUND_Stream_SetSynchCallback: function (Stream: PFSoundStream; Callback: TFSoundStreamCallback; UserData: Integer): ByteBool; {$IFDEF LINUX} cdecl {$ELSE} stdcall {$ENDIF};
  FSOUND_Stream_AddSynchPoint: function (Stream: PFSoundStream; PCMOffset: Integer; UserData: Integer): Integer; {$IFDEF LINUX} cdecl {$ELSE} stdcall {$ENDIF};
  FSOUND_Stream_DeleteSynchPoint: function (Stream: PFSoundStream; Index: Integer): ByteBool; {$IFDEF LINUX} cdecl {$ELSE} stdcall {$ENDIF};
  FSOUND_Stream_GetNumSynchPoints: function (Stream: PFSoundStream): Integer; {$IFDEF LINUX} cdecl {$ELSE} stdcall {$ENDIF};
...
var
  ...
  FMUSIC_LoadSongEx: function (Name_Or_Data: Pointer; MemLength: Integer; Mode: Cardinal; var SampleList: Integer; SampleListNum: Integer): PFMusicModule; {$IFDEF LINUX} cdecl {$ELSE} stdcall {$ENDIF};
...
{$ENDIF}
  ...
  FSOUND_Stream_AddSynchPoint     := GetAddress(FMODHandle, '_FSOUND_Stream_AddSynchPoint@12');
  FSOUND_Stream_DeleteSynchPoint  := GetAddress(FMODHandle, '_FSOUND_Stream_DeleteSynchPoint@8');
  FSOUND_Stream_GetNumSynchPoints := GetAddress(FMODHandle, '_FSOUND_Stream_GetNumSynchPoints@4');
  ...
  FMUSIC_LoadSongEx               := GetAddress(FMODHandle, '_FMUSIC_LoadSongEx@20');
