...
void display(void) {
    ...
    GLfloat light_position[] = {-10.0, 20, 10.0, 1.0};
    ...
    gluLookAt(-10.0, 45.0, 100.0,   /* eye is at (0,0,5) */
              0.0, 0.0, 0.0,    /* center is at (0,0,0) */
              0.0, 1.0, 0.);    /* up is in positive Y direction */
    ...
    glPushMatrix();
    if ( wii != NULL && wii->update()){
        double ax,ay,az;
        double _ax,_ay,_az;
        wii->getAcceleration(ax,ay,az,0);
        //glRotatef(ax ,0.0,1.0,0.0);
        //printf("ax = %0.5f, ay = %0.5f, az = %0.5f\n",ax,ay,az);
        _ax = atan2(ay,az) * 180 / M_PI;
        //_ay = atan2(ax,ay) * 180 / M_PI;
        _az = atan2(ax,az) * 180 / M_PI;
        //if (az > 0) ay += 180.0;
        //else ay = 0.0; // 180.0 - ay * 90.0;
        //glRotatef(ay,1.0,0.0,0.0);
        //glRotatef(180.0 ,0.0,0.0,1.0);

        glRotatef(180.0 - _ax,1.0,0.0,0.0);
        //glRotatef(_ay,0.0,1.0,0.0);
        glRotatef(_az,0.0,0.0,1.0);

//        glRotatef(180.0,1.0,0.0,0.0);
//        double a, angle,_ay,_az;
//        a = sqrt(ay * ay + az * az);
//        _az = az / a;
//        _ay = -ay / a;
//        angle = acos(ax) *  180 / M_PI;
//        glRotatef(angle,0.0,_az,_ay);
///        double a, roll, pitch;
///        arv::convertaxyz2arollpitch(ax, ay, az, a, roll, pitch);
///        roll = roll * 180 / M_PI;
///        pitch = pitch * 180 / M_PI;
///
///        glRotatef(180.0 - pitch,1.0,0.0,0.0);
